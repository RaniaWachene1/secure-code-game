...
def validorder(order):
    ...
    for item in order.items:
        if item.type == 'payment':
            # Check for reasonable min & max value for the invoice amounts
            if Decimal('-100000') <= Decimal(item.amount) <= Decimal('100000'):
                payments += Decimal(item.amount)
        elif item.type == 'product':
            if type(item.quantity) is int and MIN_QUANTITY <= item.quantity <= MAX_QUANTITY and Decimal('0') < Decimal(item.amount) <= Decimal(MAX_ITEM_AMOUNT):
